...
export default async function AdminContactosPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="font-serif text-3xl font-bold text-tierra">Contactos</h1>
        <p className="text-gray-500 text-sm mt-1">
          {contactos.length} mensaje{contactos.length !== 1 ? "s" : ""} en total
          {sinLeer > 0 && (
            <span className="ml-2 bg-terracota text-tierra text-xs font-bold px-2 py-0.5 rounded-full">
              {sinLeer} sin leer
            </span>
          )}
        </p>
          ...
          contactos.map((contacto) => (
            <div
              ...
              className={`bg-white rounded-xl p-6 shadow-sm border transition-colors ${
                contacto.leido ? "border-gray-200" : "border-terracota bg-yellow-50/30"
              }`}
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    {!contacto.leido && (
                      <span className="w-2 h-2 bg-terracota rounded-full flex-shrink-0" />
                    )}
                    <h3 className="font-semibold text-tierra">{contacto.nombre}</h3>
                  </div>
                  <div className="flex flex-wrap gap-3 text-sm text-gray-500 mb-3">
                    <a href={`mailto:${contacto.email}`} className="hover:text-verde-vida transition-colors">
                      {contacto.email}
                    </a>
                    {contacto.telefono && (
                      <a href={`tel:${contacto.telefono}`} className="hover:text-verde-vida transition-colors">
                        {contacto.telefono}
                      </a>
                    )}
                    <span>{formatDate(contacto.createdAt)}</span>
                    {contacto.propiedadId && (
                      <span className="text-xs bg-salvia text-verde-vida px-2 py-0.5 rounded-full">
